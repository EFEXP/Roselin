...
class MuteWordFragment : ARecyclerFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        launch(UI) {
            async {
                LivePagedListBuilder<Int,MuteFilter>( RoselinDatabase.getInstance().muteFilterDao().getMuteWord(),50).build() }.await()
    ...
    inner class MuteWordAdapter : CalculableAdapter<MuteFilter>(R.layout.item_mute) {
